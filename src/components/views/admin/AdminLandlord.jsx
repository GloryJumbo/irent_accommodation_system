...
function AdminLandlords() {
    ...
    const deleteLandlord = (e) => {
        ...
        if (e.target.id === "" || e.target.id === null || e.target.id === undefined) {
            ...
            axios.delete("http://localhost:8080/irent/api/delete/landlord/"+parseInt(e.target.id)).then(res=>{
               console.log(res);
            })
        }
    }
                            ...
                            <tbody>
                                {
                                    output.map((land, key) => {
                                        return <tr>
                                            ...
                                            <td>{land.sex}</td>
                                            <td>{land.location}</td>
                                            <td ><span onClick={deleteLandlord} id={land.id} className='badge p-2 badge-pills bg-danger'>Delete</span></td>
